...
private:
    ...
    // Stream slicing — m_streamGroup is checkable; no separate QCheckBox member
    ...
    // ── Stream slicing group
    // The QGroupBox itself is checkable — its checked state IS the enable toggle.
    // No separate QCheckBox member needed.
    QGroupBox* buildStreamGroup()
    {
        m_streamGroup = new QGroupBox("Video Slicing");
        m_streamGroup->setCheckable(true);   // the title checkbox IS the enable toggle
        m_streamGroup->setChecked(false);    // disabled / collapsed by default
        ...
        // Always visible — user ticks the title checkbox to enable slicing
    ...
    void connectSignals()
    {
        ...
        // Stream group checkable toggle → update preview
        connect(m_streamGroup, &QGroupBox::toggled, this, [this]{
            updatePreview();
        });
    ...
    {
        // Stream group is always visible; its checkable title controls enable state.
        // Nothing to sync here — kept for future use.
    }
    ...
    void updatePreview()
    {
        ...
        // Emit slice flag only when the group checkbox is ticked
        if (m_streamGroup->isChecked())
    ...
    PipelineConfig buildConfig() const
    {
        ...
        // ── Stream slicing — only active when group is visible + checked ──────
        cfg.segmentEnabled = (m_streamGroup -> isChecked());
        cfg.segmentDurationSeconds = (m_streamGroup->isChecked())
                                     ? m_sliceSpin->value()
                                     : 0;
